...
def get_pr_diff(pr_data):
  """Extract PR diff from PR data."""
  pr_diff = ""
  diff_parts =[]
  for f in pr_data.get('files',[]):
      patch = f.get('patch', '')
      if patch:
          # 模拟 Git Unified Diff 格式的文件头，方便大模型阅读
          diff_parts.append(f"--- a/{f['old_path']}\n+++ b/{f['new_path']}\n{patch}")
  
  # 将所有文件的 diff 拼成一个完整的 PR diff 字符串
  if diff_parts:
      pr_diff = "\n".join(diff_parts)
  return pr_diff
...
def get_security_audit_prompt(pr_data, pr_diff=None, include_diff=True, custom_scan_instructions=None):
    ...
    # 【新增逻辑】如果未传入 pr_diff 且需要包含 diff，则从 pr_data 中自动拼接
    if pr_diff is None and include_diff:
      pr_diff = get_pr_diff(pr_data)
